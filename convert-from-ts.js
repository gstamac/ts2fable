...
function addWithImports(filePath, imports, visited) {
    ...
    ts.forEachChild(sourceFile, node => {
        ...
        if ((imports.indexOf(importFilename) < 0) && fs.existsSync(importFilename)) {
            addWithImports(importFilename, imports, visited);
        } else {
            ...
            ts.forEachChild(exportSourceFile, node1 => {
                ...
                addWithImports(importFilename2, imports, visited);
                if (importElements) 
...
var tsFiles = config.files.map(f => {
    if (f.filename)
        ...
        return shell.ls(f.include).filter(fn => !f.exclude || !(new RegExp(f.exclude).exec(fn)));
    else    
        return undefined;
}).filter(f => f).reduce((flat, next) => flat.concat(next), []);
...
shell.find('converted').filter(fn => shell.test('-d', fn) && (shell.ls(fn).length == 0)).forEach(deleteDir);